...
const g = (over: Partial<LimitResetGrant>): LimitResetGrant => ({
  id: "grant-a", resetsLeft: 1, endsAt: 1_792_684_800, clears: ["five_hour", "seven_day"], clearsOther: false,
...
describe("publicLimitResets", () => {
  it("summarises the next grant without exposing any grant id", () => {
    ...
    expect(view).toEqual({ eligible: true, available: 3, usableNow: true, requiresLimit: false, useBy: 1_792_684_800, clears: ["five_hour", "seven_day"], clearsOther: false });
  ...
  it("says when the next grant refills windows it cannot name", () => {
    expect(publicLimitResets({ eligible: true, grants: [g({ clears: [], clearsOther: true })], nextGrantId: "grant-a", cooldownUntil: 0 }))
      .toMatchObject({ clears: [], clearsOther: true });
  });
  it("reports an ineligible account with no usable grant", () => {
    expect(publicLimitResets({ eligible: false, ineligibleReason: "cli_version", grants: [], cooldownUntil: 0 }))
      .toEqual({ eligible: false, ineligibleReason: "cli_version", available: 0, usableNow: false, requiresLimit: true, useBy: 0, clears: [], clearsOther: false });
  });
});
