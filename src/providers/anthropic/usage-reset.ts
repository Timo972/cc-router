...
export type ClaudeResetCode = "reset" | "already_used" | "not_limited" | "cooldown" | "ineligible";
export interface ClaudeResetResult { code: ClaudeResetCode; resetsLeft?: number }

const CODES: readonly ClaudeResetCode[] = ["reset", "already_used", "not_limited", "cooldown", "ineligible"];
// Claude Code maps these to "Couldn't confirm the reset went through" and keeps
// the claim for a same-id retry; so do we, whatever `result` says.
const UNCONFIRMED_REASONS: readonly string[] = ["reset_unconfirmed", "stamp_indeterminate"];
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (body && typeof body === "object" && "result" in body) {
        ...
        // `unavailable` is not in CODES: an unconfirmed claim is never "nothing used".
        if (typeof reason === "string" && UNCONFIRMED_REASONS.includes(reason)) throw new Error(RESET_OUTCOME_UNKNOWN);
